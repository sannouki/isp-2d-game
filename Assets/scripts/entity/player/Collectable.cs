    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.name == "Player") {
            if (itemType == ItemType.Coin) {
                ...
                newPlayer.IncreaseCoinsCollected();
        ...
        else if (itemType == ItemType.Ammo)
        {
                ...
                int ammoRangeValue = Random.Range(1, 6);
                newPlayer.maxAmmo += ammoRangeValue;
                if(newPlayer.maxAmmo >= 36)
                {
                    newPlayer.maxAmmo = 36;
                    Debug.Log("Collected: is full");
                }
                Debug.Log("Collectable: ammo: " + ammoRangeValue);
            }
        /*
